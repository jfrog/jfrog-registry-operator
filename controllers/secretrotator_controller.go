...
func (r *SecretRotatorReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	r.Log.Info("Starting Artifactory Secret Rotation Reconciler")
	...
	if err != nil {
		...
		r.Log.Error(err, "Failed to get SecretRotator")
		return operations.StopReconciliation()
	}

	// InitializeResource initializes the secret rotator object and validates specs
	if reconciles, err := r.InitializeResource(ctx, &tokenDetails, secretRotator, req); err != nil {
		r.Log.Error(err, "failed to initialize secret rotator object")
	...
	if reconciles, err := r.ManagingSecrets(ctx, &tokenDetails, secretRotator, req); err != nil {
		r.Log.Error(err, "failed to manage secret token")
		return reconciles, err
	}

	// UpdateStatus, update the custom resource status
	if reconciles, err := r.UpdateStatus(ctx, &tokenDetails, secretRotator); err != nil {
		r.Log.Error(err, "failed to update status")
		return reconciles, err
	}

	return ctrl.Result{RequeueAfter: tokenDetails.RefreshInt}, nil
}
